...
from module.gcn.st_gcn import Model,Model_Linear
import os
...
class RecognitionProcessor(BaseProcessor):
    ...
    def load_model(self,train_mode,weight_path,in_channels,hidden_channels,hidden_dim,
                    dropout,graph_args,edge_importance_weighting, people_importance_weighting,graph_args_1):
    ...
    def train_batch(self, data, label):
        N = data.size(0)
        tensor = torch.zeros(N, 12, dtype=torch.int)
        for i in range(N):
            row = torch.arange(12, dtype=torch.int)
            tensor[i] = row
...
def local_mix_regularization(inputs_1, inputs2_1, mix_type, beta):
    if mix_type in ['mixup', 'cutmix', 'framemix', 'cubemix', 'framemixup', 'fademixup', 'cutmixup', 'cubemixup']:
        # Sample Mix Ratio (Lambda)
        ls = [2,3,4]
        inputs = inputs_1[:,:,:,:,ls]
...
def local_skeleton_mix_regularization(inputs_1, inputs2_1, mix_type, beta):
    if mix_type in ['mixup', 'cutmix', 'framemix', 'cubemix', 'framemixup', 'fademixup', 'cutmixup', 'cubemixup']:
        # Sample Mix Ratio (Lambda)
...
def local_temporal_mix_regularization(inputs_1, inputs2_1, mix_type, beta):
    if mix_type in ['mixup', 'cutmix', 'framemix', 'cubemix', 'framemixup', 'fademixup', 'cutmixup', 'cubemixup']:
        # Sample Mix Ratio (Lambda)
        ls = [0,3,6,9]
        inputs = inputs_1[:,:,ls,:,:]
...
def global_mix_regularization1(inputs_1, inputs2_1, mix_type, beta):
    shear = inputs_1.clone()
    crop = inputs2_1.clone()
    out = shear.clone()
    if mix_type in ['cubemixup']:
        ...
        # Random Mix within Batch
        rand_index = torch.randperm(shear.size()[0]).cuda()
        ...
        if mix_type in ['cubemixup']:
            ...
            if mix_type in ['cubemixup']:
                mix_tmp = shear * lamt + crop * (1. - lamt)
                fr = np.random.rand(1)
                if fr < 0.5:  # Basic MixUp, 0.5 Prob FrameMixUp
                    if lamt >= 0.5:
                        # TODO: 这里的 rand-index 会把之前的顺序给打乱，mix的数据可能就不是原来的数据了，性能可能会下降，跑的时候要做好记录
                        out[:, :, bbt1:bbt2, bbx1:bbx2, bby1:bby2] = mix_tmp[rand_index, :, bbt1:bbt2, bbx1:bbx2,
                                                                         bby1:bby2]

                    else:
                        out[:, :, bbt1:bbt2, bbx1:bbx2, bby1:bby2] = mix_tmp[:, :, bbt1:bbt2, bbx1:bbx2, bby1:bby2]

    return out, lamt, shear, crop
...
class BTProcessor(BaseProcessor):
    ...
    def train_epoch(self, epoch, pretrain_epoch, pretrain_lr):
        ...
        loader = self.data_loader['train']
        self.adjust_learning_rate(self.optimizer, current_epoch=epoch, max_epoch=pretrain_epoch, lr_min=pretrain_lr*0.0001, lr_max=pretrain_lr)
        ...
        for data, label in tqdm(loader):
            ...
            input3_1 = shear(data)
            input3_2 = crop(data)
            input3, lamt, input3_crop, input3_shear, lam = global_mix_regularization_lambdasame(input3_1, input3_2,
                                                                                                'cubemixup', 1.2)
            ...
            subgroup_feat3 = feat3.gather(1, people_list)
            subgroup_feat2 = feat2.gather(1, people_list)
            subgroup_feat3_1 = feat3_1.gather(1, people_list)
            subgroup_feat3_2 = feat3_2.gather(1, people_list)
            subgroup_mean_feat3 = subgroup_feat3.mean(dim=1)
            subgroup_mean_feat2 = subgroup_feat2.mean(dim=1)

            subgroup_encoded_feat3_2 = encoded_feat3_2.gather(1, people_list)
            subgroup_encoded_feat3_1 = encoded_feat3_1.gather(1, people_list)
            subgroup_encoded_feat3 = encoded_feat3.gather(1, people_list)
            subgroup_encoded_feat2 = encoded_feat2.gather(1, people_list)
            subgroup_mean_encoded_feat2 = subgroup_encoded_feat2.mean(dim=1)
            subgroup_mean_encoded_feat3 = subgroup_encoded_feat3.mean(dim=1)
            #
            ...
            perm = torch.randperm(subgroup_feat3.size(0)).cuda()
            subgroup_feat3_shuffled = subgroup_feat3.index_select(0, perm)
            hyperbolic = dist(subgroup_feat3, subgroup_feat3_shuffled, w=torch.tensor(1.), dim=1)

            remain_hyperbolic = 1 - hyperbolic
            remain_hyperbolic = remain_hyperbolic.unsqueeze(1)
            hyperbolic = hyperbolic.unsqueeze(1)
            mix_cos_subgroup_feat3 = hyperbolic * subgroup_feat3_shuffled + remain_hyperbolic * subgroup_feat3
            mix_cos_subgroup_feat3_mean = mix_cos_subgroup_feat3.mean(dim=1)
            # print(subgroup_mean_feat2.shape, mix_cos_subgroup_feat3.shape)
            loss = loss + self.btwins_batch(subgroup_mean_feat2, mix_cos_subgroup_feat3_mean, mode='temp_mask')
            ...
            for i in range(subgroup_feat2.size(1)):

                single_feat2 = subgroup_feat2[:, i, :]
                encoded_single_feat2 = subgroup_encoded_feat2[:, i, :]
                single_feat3_1 = subgroup_feat3_1[:, i, :]
                single_feat3_2 = subgroup_feat3_2[:, i, :]
                encoded_single_feat3_2 = subgroup_encoded_feat3_2[:, i, :]
                encoded_single_feat3_1 = subgroup_encoded_feat3_1[:, i, :]
                # encoded_single_feat3 = subgroup_encoded_feat3[:,i,:]
                loss_encoded_ps_3_1 = self.btwins_batch(encoded_single_feat3_1, subgroup_mean_encoded_feat3,
                                                        mode='temp_mask')
                loss_encoded_ps_3_2 = self.btwins_batch(encoded_single_feat3_2, subgroup_mean_encoded_feat3,
                                                        mode='temp_mask')
                if lam < 0.5:
                    loss_encoded_ps_3 = loss_encoded_ps_3_1 * (lam * lamt + (1 - lam)) + loss_encoded_ps_3_2 * (
                                (1 - lam) * lamt)
                else:
                ...
                loss_encoded_ps1 = self.btwins_batch(encoded_single_feat2, subgroup_mean_encoded_feat3,
                                                     mode='temp_mask')
                ...
                loss_ps_3_1 = self.btwins_batch(single_feat3_1, subgroup_mean_feat3, mode='temp_mask')
                loss_ps_3_2 = self.btwins_batch(single_feat3_2, subgroup_mean_feat3, mode='temp_mask')
                if lam < 0.5:
                ...
                loss_ps1 = self.btwins_batch(single_feat2, subgroup_mean_feat3, mode='temp_mask')
